...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E> implements
	...
	protected void recDisplay(Position<E> current, 
			int[] control, int level) 
	{
		...
		if (this.isRoot(current))
			System.out.println("__ROOT("+current.getElement()+")");
		else if (this.left(this.parent(current)) == current)
			System.out.println("__L("+current.getElement()+")");
		else 
			System.out.println("__R("+current.getElement()+")");
		control[level]--;
		int nc = this.numChildren(current);
		control[level+1] = nc;
		for (Position<E>  p : this.children(current)) {
